...
<!DOCTYPE html>
    ...
    <?php if (!isset($_SESSION['user_level'])) header("location: ../login/loginn.php");  ?>
        ...
        <div style ="background-color:#81559d;box-shadow: 5px 5px #4a4a4a;">
            <div class="text" style ="font-size:30px;color:white;">รับเรื่องแจ้งซ่อม</div style="padding:30px;"></div>
        </div><br>
        ...
        <div style="padding:20px">
            ...
            <form action="index_take.php" method="POST">
            ...
            <table class="table" id="data_table">
                <thead>
                    <tr>
                        ...
                        <th style="text-align: center">รับเรื่อง</th>
                        </tr>
                </thead>
                <tbody>
        ...
        $sql =  "SELECT * FROM `notify_repair` where  status_repair ='รอดำเนินการ' AND type_work='$user_level' \n". "ORDER BY `notify_repair`.`day_take` DESC;";
                ...
                while($row = mysqli_fetch_assoc($result_total)) {
                    ...
                    <tr>
                          ...
                          <td style ='text-align:center;'>
                              <a onclick="send_repair('<?php echo  $row['id'] ?>,<?php echo $user ?>')" helf='#'>
        ...
        <script>
            function send_repair(id,user) {
                Swal.fire({
                    title: 'รับเรื่อง',
                    text: "คุณต้องส่งเรื่องให้ช่างซ่อมหรือไม่",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        ...
                        setTimeout(function() {
                            window.location = "../backend/qury_user/send_repair.php?take_fix=" + id +"&user="+ user;
                        }, 2000);
